...
本阶段必须在正式发布前完成，目标是把技术流程整理成清晰的产品流程，并补齐素材管理、登录凭证和安全说明。

#### 右侧主页的三步主流程

在右侧主内容区加入步骤导航。横向步骤条（可直接导航）与竖向折叠步骤条（整版展开/收起）二选一，不同时维护两套交互；切换步骤不能清空已有状态。

主流程为三步：

1. **选择素材来源**：本步骤不可跳过，用户必须从以下三个入口中选择一个：
   - **微信导入**：选择本机微信账号并导入其收藏素材。
   - **本机导入**：从已有文件夹或单张/批量图片导入；界面同时提示“更多应用待支持”。
   - **使用已添加素材**：不执行新的导入，直接从 App 已管理的素材中继续选择，并准备导出或转移到目标应用。
2. **连接目标 App**：首发目标只有 WhatsApp，在此完成凭证存储选择、连接状态检查、扫码/配对及必要的重连操作。结构和文案允许以后增加其他目标，但暂不开发插件系统。
3. **选择并准备贴纸**：显示当前选择的图片，让用户筛选、调整顺序并查看分包结果，再进入发送流程。

步骤之间应允许返回和修改，且不能因为切换步骤丢失导入结果、选择、顺序、WhatsApp session 或尚未发送的分包状态。

#### 素材库、来源分组与筛选

- 在侧边栏增加一个专门的素材库 tab，用于浏览已经由本 App 管理的全部图片。
- 素材库支持预览、选择和排序，与第三步共用一份数据；第三步只聚焦本次准备发送的素材。
...
Phase 8 验收至少包括：三步流程可完整往返且状态不丢失；三个素材来源入口可用；素材库可预览、排序和按账号/来源筛选；第三步显示本次选中图片；两种凭证模式分别完成首次登录和重启复用；登出后 session 确实删除并要求重新扫码；应用内安全 FAQ 可访问，官网文案已准备供 Phase 9 复用。
