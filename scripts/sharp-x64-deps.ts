/**
 * Phase 5 packaging helper for sharp's platform-specific binaries.
 *
...
function stashArm64Packages(): void {
  for (const name of arm64PackageNames) {
    ...
    if (existsSync(source)) {
      const stashed = join(nodeModules, '.phase5-stash', name)
...
function clean(): void {
  ...
  rmSync(join(nodeModules, '.phase5-stash'), { recursive: true, force: true })
  if (!isArm64Present()) {
